...
pub fn pokemon_encoder(pokemon: Pokemon) -> String {
  json.object([
    ...
    #("stats", json.object(list.map(pokemon.stats, stat_entry_to_pair))),
...
pub type AbilityInfo {
  AbilityInfo(name: String, url: String)
}
...
pub type AbilityEntry {
  AbilityEntry(ability: AbilityInfo, is_hidden: Bool, slot: Int)
}
...
pub type VersionInfo {
  VersionInfo(name: String, url: String)
}
...
pub type GameIndexEntry {
  GameIndexEntry(game_index: Int, version: VersionInfo)
}
...
pub type StatInfo {
  StatInfo(name: String, url: String)
}
...
pub type StatEntry {
  StatEntry(base_stat: Int, stat: StatInfo)
}
